...
from src.utils import get_lr, do_keep_cell_fp32, load_ckpt, build_params_groups, context_device_init, count_params
from src.metrics import DistAccuracy, ClassifyCorrectCell
from src.callbacks import Monitor
from src.cell_wrapper import CustomTrainOneStepCell, CustomTrainOneStepWithLossScaleCell
...
def train():
    # config.pretrain_ckpt = os.path.join(config.data_path, config.pretrain_ckpt)
    ...
    config.lr_max = 0.00005
    config.lr_min = 0.00005
    config.lr_warmup_epochs = 0
    config.weight_decay = 0.00000001

    config.enable_ema = True

    # Set task run parameter manually
    # config.ema_decay = 0.99995
